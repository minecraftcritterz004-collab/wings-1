...
const StepsCard = () => {
  return (
    <div className="relative bg-white/80 backdrop-blur-sm rounded-lg mb-2 border-2 !border-[rgb(207,16,45)] shadow-2xl overflow-hidden">
        <div
          className="absolute inset-0 bg-center bg-no-repeat bg-cover pointer-events-none opacity-10"
        >
        </div>
      <ol className="relative z-10 text-left space-y-1 text-black p-2.5 !text-base !rounded-none">
        {steps.map((step, index) => {
          const Icon = step.icon;
          return (
            <li key={index} className="flex items-start gap-1.5 text-[11px] md:text-xs">
                    <span className="flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center font-bold text-[9px] mt-0.5 text-white" style={{ backgroundColor: '#E21E26', color: '#ffffff' }}>
                      {index + 1}
                    </span>
              <div className="flex items-center gap-1 flex-1">
                <span className="flex-shrink-0" style={{ color: '#E21E26' }}>
